import { Array, Effect, FiberMap, Layer, Option, Ref, Schema } from "effect";
	...
	Effect.gen(function* () {
		...
		return {
			Cleanup: Effect.fn("@naamio/mercury/ConversationGenerationManager#Cleanup")(function* () {
				...
				yield* FiberMap.clear(ongoingGenerations);
			}),
			InterruptGeneration: Effect.fn("@naamio/mercury/ConversationGenerationManager#InterruptGeneration")(
				function* (envelope) {
					yield* FiberMap.remove(ongoingGenerations, envelope.payload.messageId);
				},
			),
			StartGeneration: Effect.fn("@naamio/mercury/ConversationGenerationManager#StartGeneration")(function* (envelope) {
				...
				const hasOngoingGeneration = yield* FiberMap.has(ongoingGenerations, envelope.payload.messageId);

				if (hasOngoingGeneration) {
